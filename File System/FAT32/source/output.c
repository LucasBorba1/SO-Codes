...
/* Função para formatar valores em B, KiB ou MiB */
...
void show_files(struct fat_dir *dirs)
{
    ...
    while ((cur = dirs++) != NULL)
    {
        ...
        else if (cur->attr == 0xf) /* Ignorar entradas de Long File Name (LFN) */
            continue;
...
/* Mostra informações detalhadas do BPB */
void verbose(struct fat_bpb *bios_pb)
{
    ...
    /* Alteração: Adiciona suporte para FAT32 */
    fprintf(stdout, "Large number of sectors: %d\n", bios_pb->large_n_sects);
    fprintf(stdout, "Sectors per FAT (FAT32): %d\n", bios_pb->sect_per_fat32);
    fprintf(stdout, "Root cluster (FAT32): 0x%x\n", bios_pb->root_cluster);
    fprintf(stdout, "File system version (FAT32): %d\n", bios_pb->fs_version);
    fprintf(stdout, "FSInfo sector (FAT32): %d\n", bios_pb->fs_info);
    fprintf(stdout, "Backup boot sector (FAT32): %d\n", bios_pb->boot_sector_backup);

    fprintf(stdout, "Sector per track: %d\n", bios_pb->sect_per_track);
    fprintf(stdout, "Number of heads: %d\n", bios_pb->number_of_heads);

    /* Endereços calculados usando funções de suporte */
